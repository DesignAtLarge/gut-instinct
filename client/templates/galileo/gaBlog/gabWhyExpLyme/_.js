...
Template.gabWhyExpLyme.onCreated(function() {
    Session.setDefault('isDemo', true);
    let inst = Template.instance();
    inst.form = new ReactiveVar(undefined);

    Meteor.call('galileo.boards.getGoogleForm', "LYME", function(err, result) {
        inst.form.set(result)
        $("title").html("Galileo | Why Exp Lyme");
    });

});

Template.gabWhyExpLyme.helpers({
    "form": function() {
        return Template.instance().form.get();
    }
});

Template.gabWhyExpLyme.events({
