...
Template.gabWhyExpNerdNite.onCreated(function() {
    Session.setDefault('isDemo', true);
    let inst = Template.instance();
    inst.form = new ReactiveVar(undefined);

    Meteor.call('galileo.boards.getGoogleForm', "NERDNITE", function(err, result) {
        inst.form.set(result)
        $("title").html("Galileo | Why Exp Nerd Nite");
    });

});

Template.gabWhyExpNerdNite.helpers({
    "form": function() {
        return Template.instance().form.get();
    }
});

Template.gabWhyExpNerdNite.events({
