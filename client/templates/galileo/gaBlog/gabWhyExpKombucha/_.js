...
Template.gabWhyExpKombucha.onCreated(function() {
    Session.setDefault('isDemo', true);

    let inst = Template.instance();
    inst.form = new ReactiveVar(undefined);

    Meteor.call('galileo.boards.getGoogleForm', "KOMBUCHA", function(err, result) {
        inst.form.set(result)
        $("title").html("Galileo | Why Exp Kombucha");
    });
});

Template.gabWhyExpKombucha.helpers({
    "form": function() {
        return Template.instance().form.get();
    }
});

Template.gabWhyExpKombucha.events({
