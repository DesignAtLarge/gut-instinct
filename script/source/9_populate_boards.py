...
def populate_test(db):
    print "Populating Board Items in DB ..."
    db.ga_boards.delete_many({});
    f = open(os.path.dirname(os.path.realpath(__file__)) + '/galileo_boards.json')
    board = json.loads(f.read())
    for items in board:
        print("inserting item: " + str(items))
        db.ga_boards.insert(items)
