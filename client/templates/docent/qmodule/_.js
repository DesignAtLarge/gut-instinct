...
Template.qmodule.onCreated(function() {
...
Template.qmodule.rendered = function() {
    $('.chips-initial').hide();
    ...
    isInFlaggedQues: function() {
        ...
        try {
            if (Meteor.user()) {
                ...
                var fetchResult = FlaggedQues.findOne({
                    "typeid": insertTypeID
                });
...
Template.qmodule.events({
        ...
        if ($(event.target).parent().find(".followUpAddOptionInput").css('display') != 'none' && checkedLength == 0 && userAddOptionValue === '') {
            ...
            if (userAddOptionValue != '') {
                ...
                if (checkedLength > 0) {
                    ...
                    userResponseArr[userResponseArr.length] = totalLength; /* now that we hide the option box + 1*/
                } else {
                    userResponseArr[0] = totalLength;
                }
                ...
                var currentNewOption = {
                    ...
                    "option_text": userAddOptionValue,
                    "option_index": totalLength,
                    /* now that we hide the option box + 1*/
    ...
    'click .questionAddFlagged': function(event) {
        searchid = this._id;
        if (typeof(searchid) === 'object') {
            var insertTypeID = searchid.toString().match(/"([^"]+)"/)[1];
        } else {
    ...
    'click .questionRemoveFlaggedExpert': function(event) {
        searchid = this._id;
    ...
    'click .questionRemoveFlaggedUser': function(event) {
        Materialize.toast("Once a question is flagged, only experts can unflag it after review", 2000, 'toast')
    },
            ...
            Meteor.call('profile.getNotificationArrayOfUser', currentQuestionOwner.username, function(err, result) {
                if (result && result[4] == 1) {
                    var noticeID = Notifications.insert({
                        ...
                        raw_owner_name: searchOwnerName,
                        type: 'feedback from scientific material',
                        created_at: created_at,
                        typeid: inst.hash,
                        typestring: noticeString,
                        isRead: 0
