...
Template.gabWhyExpBeer.onCreated(function() {
    Session.setDefault('isDemo', true);

    let inst = Template.instance();
    inst.form = new ReactiveVar(undefined);

    Meteor.call('galileo.boards.getGoogleForm', "BEER", function(err, result) {
        inst.form.set(result)
        $("title").html("Galileo | Why Exp Beer");
    });
});

Template.gabWhyExpBeer.helpers({
    "form": function() {
        return Template.instance().form.get();
    }
});

Template.gabWhyExpBeer.events({
